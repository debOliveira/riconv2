...
def group_index(nsample, radius, xyz, new_xyz, group='knn'):
    if group == 'knn':
        ...
        idx = pointnet2_utils.ball_query(radius, nsample, xyz,
                                         new_xyz.contiguous())
        idx = idx.long()
    else:
...
def order_index(xyz, new_xyz, new_norm, idx):
    ...
    projected_xyz_unit[projected_xyz_unit !=
                       projected_xyz_unit] = 0  # set nan to zero

    length_max_idx = torch.argmax(proj_xyz_length, dim=2)
    vec_ref = projected_xyz_unit.gather(
        2,
        length_max_idx.unsqueeze(-1).repeat(
            1, 1, 1, 3))  # corresponds to the largest length
    ...
    sign[:, :, 0,
         0] = 1.  # the first is the center point itself, just set sign as 1 to differ from ref_vec
...
def RI_features(xyz, norm, new_xyz, new_norm, idx, group_all=False):
    ...
    grouped_xyz_length = torch.norm(grouped_xyz_local, dim=-1,
                                    keepdim=True)  # nn lengths
    grouped_xyz_unit = grouped_xyz_local / grouped_xyz_length
    grouped_xyz_unit[grouped_xyz_unit !=
                     grouped_xyz_unit] = 0  # set nan to zero
    grouped_xyz_norm = index_points(norm,
                                    idx_ordered)  # nn neighbor normal vectors

    grouped_xyz_angle_0 = torch.matmul(grouped_xyz_unit, new_norm)
    grouped_xyz_angle_1 = (grouped_xyz_unit * grouped_xyz_norm).sum(
        -1, keepdim=True)
    grouped_xyz_angle_norm = torch.matmul(grouped_xyz_norm, new_norm)
    grouped_xyz_angle_norm = torch.acos(
        torch.clamp(grouped_xyz_angle_norm, -1 + epsilon, 1 - epsilon))  #
    D_0 = (grouped_xyz_angle_0 < grouped_xyz_angle_1)
    D_0[D_0 == 0] = -1
    grouped_xyz_angle_norm = D_0.float() * grouped_xyz_angle_norm

    grouped_xyz_inner_vec = grouped_xyz_local - torch.roll(
        grouped_xyz_local, 1, 2)
    grouped_xyz_inner_length = torch.norm(grouped_xyz_inner_vec,
                                          dim=-1,
                                          keepdim=True)  # nn lengths
    grouped_xyz_inner_unit = grouped_xyz_inner_vec / grouped_xyz_inner_length
    grouped_xyz_inner_unit[grouped_xyz_inner_unit !=
                           grouped_xyz_inner_unit] = 0  # set nan to zero
    grouped_xyz_inner_angle_0 = (grouped_xyz_inner_unit *
                                 grouped_xyz_norm).sum(-1, keepdim=True)
    grouped_xyz_inner_angle_1 = (grouped_xyz_inner_unit *
                                 torch.roll(grouped_xyz_norm, 1, 2)).sum(
                                     -1, keepdim=True)
    grouped_xyz_inner_angle_2 = (grouped_xyz_norm *
                                 torch.roll(grouped_xyz_norm, 1, 2)).sum(
                                     -1, keepdim=True)
    grouped_xyz_inner_angle_2 = torch.acos(
        torch.clamp(grouped_xyz_inner_angle_2, -1 + epsilon, 1 - epsilon))
    ...
    ri_feat = torch.cat([
        grouped_xyz_length, proj_inner_angle_feat, grouped_xyz_angle_0,
        grouped_xyz_angle_1, grouped_xyz_angle_norm, grouped_xyz_inner_angle_0,
        grouped_xyz_inner_angle_1, grouped_xyz_inner_angle_2
    ],
                        dim=-1)
...
def sample_and_group_all(xyz, norm):
    ...
    # create ri features
    ri_feat, idx_ordered = RI_features(xyz,
                                       norm,
                                       new_xyz,
                                       new_norm,
                                       idx,
...
class RIConv2SetAbstraction(nn.Module):
    ...
    def forward(self, xyz, norm, points):
        ...
        if self.group_all:
            new_xyz, ri_feat, new_norm, idx_ordered = sample_and_group_all(
                xyz, norm)
        else:
            new_xyz, ri_feat, new_norm, idx_ordered = sample_and_group(
...
class RIConv2FeaturePropagation_v2(nn.Module):
    ...
    def forward(self, xyz1, xyz2, norm1, norm2, points1, points2):
        ...
        ri_feat, idx_ordered = sample_and_group_deconv(self.nsample, xyz2,
                                                       norm2, xyz1, norm1)
        ri_feat = ri_feat.permute(0, 3, 2, 1)
        for i, conv in enumerate(self.prev_mlp_convs):
            bn = self.prev_mlp_bns[i]
        ...
        if points2 is not None:
            ...
            grouped_points = grouped_points.permute(0, 3, 2, 1)
            new_points = torch.cat([ri_feat, grouped_points],
